## 👋 Oi! Eu sou a Julia
-  Estudante de Bacharelado Interdisciplinar em Ciência e Tecnologia   (UNIFESP)
-  Pretendo seguir Ciência da Computação  


Atualmente estou construindo minha base em programação, estou no terceiro semestre do bacharelado, nos períodos anteriores fiz duas disciplinas essenciais para o aprendizado de computação: Lógica de Programação e Algoritmos e Estruturas de Dados 1, ambas em linguagem C/C++.

---

### 📚 Formação e aprendizado
- Bacharelado em Ciência e Tecnologia – UNIFESP (em andamento)
- Curso de Introdução ao Python – USP
- Curso de Machine Learning com Python – USP (em andamento)

---

### 🏅 Certificações
- Microsoft AI-900 – Azure AI Fundamentals
- Microsoft PL-900 – Power Platform Fundamentals

---

### Em estudo
- C/C++
- Python
- Git & GitHub
- Linux (atualmente usando o Mint)
- Algoritmos
- Estruturas de Dados

---

### Experiência técnica
-  Eletricista de manutenção em eletroeletrônica  
  Atuação com diagnóstico e correção de falhas, leitura de esquemas elétricos,
  análise de sistemas e aplicação de procedimentos técnicos.

---

### 🚀 Objetivo
Desenvolver projetos práticos em Python e C/C++ (e logo em outras linguagens também), e projetos relacionados as disciplinas da universidade. 
